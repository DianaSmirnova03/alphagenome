...
def compute_loss(
    ...
    # sample_weight (опционально) — напр. чем ниже fdr_comb_pval (увереннее ASE-вызов),
    # тем больше вес строки в регрессионном loss. Если не передан — все веса = 1.
    sample_weight = batch.get("sample_weight") if isinstance(batch, dict) else None
    if sample_weight is None:
        sample_weight = jnp.ones_like(z_loss_per_ex)
    z_loss = jnp.sum(z_loss_per_ex * z_mask * sample_weight) / jnp.maximum(
        jnp.sum(z_mask * sample_weight), 1.0
    )
